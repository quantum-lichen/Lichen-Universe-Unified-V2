...
with st.sidebar:
    ...
    if manifest:
        st.success(f"🟢 HIVE LINK: CONNECTED (v{manifest['project']['version']})")
    else:
    ...
    if manifest:
        cst = manifest['universal_constants']['definitions']
        st.caption(f"φ = {cst['phi']['value']}")
        st.caption(f"π = {cst['pi']['value']}")
        st.caption(f"Dim = {cst['perfect_496']['value']}")
...
st.divider()
if manifest:
    st.caption(f"Generated by {manifest['project']['generator']} | {manifest['meta']['birthday_special']}")
